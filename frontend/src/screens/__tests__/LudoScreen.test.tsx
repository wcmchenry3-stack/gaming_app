...
import LudoScreen from "../LudoScreen";
...
function mockNav() {
  return {
    navigate: jest.fn(),
    goBack: jest.fn(),
  } as unknown as Parameters<typeof LudoScreen>[0]["navigation"];
}

function renderScreen(nav = mockNav()) {
  return render(
    <ThemeProvider>
      <LudoScreen navigation={nav} />
    </ThemeProvider>
  );
}
...
describe("LudoScreen — initial load", () => {
  ...
  it("renders header after newSession fails (error state)", async () => {
    ...
    await findByText("Ludo");
  });
});

describe("LudoScreen — roll", () => {
  it("calls ludoApi.roll when Roll button pressed", async () => {
    mockApi.newSession.mockResolvedValue(makeRollState());
