...
describe("fruit vertex quality", () => {
  ...
  it.each(
    fruitSet.fruits
      .filter((d) => d.icon)
      .map((d) => [nameKeyFor(d), d.name] as const)
  )("%s (%s) — hull fills at least 60%% of [-1,1] range", (key) => {
  ...
  it.each(
    fruitSet.fruits
      .filter((d) => d.icon)
      .map((d) => [nameKeyFor(d), d.name] as const)
  )("%s (%s) — vertices stay within [-1,1] bounds", (key) => {
    const verts = getVerticesForFruit("fruits", key);
    expect(verts).not.toBeNull();
  ...
  it.each(
    fruitSet.fruits
      .filter((d) => d.icon)
      .map((d) => [nameKeyFor(d), d.name] as const)
  )("%s (%s) — simplifies to ≤ 24 vertices", (key) => {
    const verts = getVerticesForFruit("fruits", key);
    expect(verts).not.toBeNull();
  ...
  it.each(
    fruitSet.fruits
      .filter((d) => d.icon)
      .map((d) => [nameKeyFor(d), d.name] as const)
  )("%s (%s) — hull centre is near origin (no large offset)", (key) => {
    const verts = getVerticesForFruit("fruits", key);
    expect(verts).not.toBeNull();
...
describe("fruit sprite alignment", () => {
  ...
  it.each(
    fruitSet.fruits
      .filter((d) => d.icon)
      .map((d) => [nameKeyFor(d), d.name] as const)
  )("%s (%s) — spriteScale is reasonable (0.8–3.0)", (key) => {
  ...
  it.each(
    fruitSet.fruits
      .filter((d) => d.icon)
      .map((d) => [nameKeyFor(d), d.name] as const)
  )("%s (%s) — spriteOffset is not extreme (< 1.0)", (key) => {
    const info = getSpriteInfo("fruits", key);
    expect(info).not.toBeNull();
...
describe("fruit hull polygon validity", () => {
  ...
  it.each(
    fruitSet.fruits
      .filter((d) => d.icon)
      .map((d) => [nameKeyFor(d), d.name] as const)
  )("%s (%s) — polygon has non-zero area (not degenerate)", (key) => {
    const verts = getVerticesForFruit("fruits", key);
    expect(verts).not.toBeNull();
  ...
  it.each(
    fruitSet.fruits
      .filter((d) => d.icon)
      .map((d) => [nameKeyFor(d), d.name] as const)
  )("%s (%s) — no duplicate adjacent vertices", (key) => {
    const verts = getVerticesForFruit("fruits", key);
    expect(verts).not.toBeNull();
